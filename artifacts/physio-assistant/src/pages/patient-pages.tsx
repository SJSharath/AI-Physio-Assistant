...
export function ExerciseInstructionsPage() {
  ...
  const prescriptionsQuery = useListPrescriptions();
  const prescription = prescriptionsQuery.data?.find((item) => item.id === prescriptionId) ?? findPrescription(prescriptionId) ?? demoPrescriptions[0];
